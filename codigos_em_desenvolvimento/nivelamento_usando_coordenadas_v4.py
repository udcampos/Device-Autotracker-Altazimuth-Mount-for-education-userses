...
lugar, a verão 4 implementa a função fim_curso"""
...
class EncontraNorte:
    ...
    def le_sensor_nivel(selt,amostras):        
        objeto=sm(amostras)
   ...
   ## Essa função corrige o nivel do telescópio, o algoritimo foi feito apos estudo matemático
    def corrige_nivel(self):
        ...
        if (diferenca>8) and (diferenca<10) and quadrado>0:
            print(diferenca,quadrado)
            return True

        elif (diferenca>10) and (diferenca<20) and quadrado>100:
            self.motor_vertical.anti_horario(10,3)
            self.corrige_nivel()

        elif diferenca>20 and diferenca<100 and quadrado>400:
            self.motor_vertical.anti_horario(10,30)
            self.corrige_nivel()

        elif diferenca<-20 and quadrado>8000:
            self.motor_vertical.anti_horario(10,220)
            self.corrige_nivel()
        elif diferenca<-50 and quadrado>20000:
            self.motor_vertical.anti_horario(10,260)                    
            self.corrige_nivel()    
        ## Condições para corrigir o nível quando o telescopio esta abaixo do nivel
        else:
            
            self.motor_vertical.horario(10,5)
            self.corrige_nivel()
        return True
    ...
    def avalia_lado_anti_horario(self):
        ...
        while contador<10:
            amostras=3
            vetor.append(int(self.le_sensor_posicao(amostras)))
            self.motor_horizontal.anti_horario(10,18)
            contador+=1
    ...
    def avalia_lado_horario(self):
        ...
        while contador<10:
            amostras=3
            vetor.append(int(self.le_sensor_posicao(amostras)))
            self.motor_horizontal.horario(10,18)
            contador+=1
    ...
    def automatiza_correcao(self):
        ...
        if posicao==True and chave==False:
            ...
            fim_de_curso=self.corrige_nivel_fim_curso()            
            ##print(nivel)
    ...
    def varias_leituras(self):
        ...
        while a<210:
            ...
            self.motor_horizontal.horario(10,25)
            print(a,",",b,",",c,",",d)
            #print(a,",",f,",",e)
            ##print(a)
            a+=1
   ...
   ## Função de ajuste da curva dos eixos yx e zx
    ...
    def aponta_norte(self):
        ...
        self.motor_horizontal.horario(10,550)
        ...
        amostras=15
        defasagem_telescopio_sensor=self.le_sensor_posicao(amostras) ## Valor pode ser alterado        
        ## A referencia e a declinação encontram o norte geografico
        while defasagem_telescopio_sensor<=declinacao:
            amostras=40
            self.motor_horizontal.horario(10,2)
