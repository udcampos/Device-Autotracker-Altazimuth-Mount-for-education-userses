...
Nota V5: - Corrigido o algulo do pulo, para retornar sempre 1 passo
         ...
         com o objetivo de corrigir as coordenadas depois do acompanhamento.
Nota V6:
    - Durante o acompanhamento o motor não consegue pular um passo, vou adequar
    para que a quantidade de passos seja adequada.
     - Melhorar a precisão do azimute. Para isso o algoritimo
    vai fazer duas verificações, 1 para posicionar e outra para corrigir caso a
    estrela tenha se deslocado.
Nota V9:
    -A função para achar a primeira estrela, foi separado em duas partes acha
    azimute e acha altura, dessa forma, será possível repetir as funções
    quantas vezes for necesário, o objetivo é buscar a primeira posição, para
    no final buscar um posicionamento mais preciso.
"""
...
# # Essa função movimenta o motor do azimute sentido anti_horario
    ...
    posicionado"""
    def analisa_posicao_estrelas(self, vetor_azimute):
    ...
    def define_azimute_proxima_estrela(self):
        ...
        e1, e2 = self.analisa_posicao_estrelas(vetor_azimute)
        if e1 == e2:
            if vetor_azimute[-2] > vetor_azimute[-1]:
                ...
                # Acrescentado para corrigir correçao, pois deleta os valores
                # Anteriores, e inicia o acompanhamento da coordenada atual
        ...
        if e2 == 2 and e1 == 1:
            if (vetor_azimute[-1] - vetor_azimute[-2]) <= 180:
                ...
                if vetor_azimute[-1] > 270 and vetor_azimute[-2] < 90:
                    ...
                    print(vetor_azimute[-2], vetor_azimute[-1])
                    print(angulo_resultante)
                    self.aciona_motor_horizontal_anti_horario
                    (angulo_resultante)
                    del self.vetor_acompanha_azimute[:-1]
                    return True
    ...
    def acompanha_estrela_azimute(self):
        ...
        self.vetor_acompanha_azimute.append(self.azimute_decimal())
        valor_correcao = (self.angulo_passo_azimute *
                          self.passos_minimos_acompanhamento)
        ...
        if self.vetor_acompanha_azimute[0] > self.vetor_acompanha_azimute[-1]:
            ...
            print("A diferença do azimute é: ",
                  (self.vetor_acompanha_azimute[0] -
                   self.vetor_acompanha_azimute[-1]))
            #  Faz uma comparação para verificar se a subtração dos
            #  valores da para movimentar um passo
            if ((self.vetor_acompanha_azimute[0] -
                 self.vetor_acompanha_azimute[-1]) < valor_correcao):
                sleep(0.01)
            else:
            ...
            if (self.vetor_acompanha_azimute[-1] -
                    self.vetor_acompanha_azimute[0]) < valor_correcao:
                sleep(0.05)
            else:
    ...
    def acompanha_estrela_altura(self):
        ...
        if self.vetor_acompanha_altura[0] > self.vetor_acompanha_altura[-1]:
            print("O valor da correção é: ", valor_correcao)
            print("A diferença da altura é: ",
                  (self.vetor_acompanha_altura[0] -
                   self.vetor_acompanha_altura[-1]))
            if (self.vetor_acompanha_altura[0] -
                    self.vetor_acompanha_altura[-1]) < valor_correcao:
                sleep(0.01)
            else:
...
# #Quando corrige os passos, é guardado a ultima posição da estrela.
    ...
    def acompanha_estrela(self):
        ...
        while memoria == nome_atual:
            ...
            print("Os passos pulados no azimute foram ",
                  self.passos_corrigidos_azimute)
            print("Os passos pulados na altura foram ",
                  self.passos_corrigidos_altura)
