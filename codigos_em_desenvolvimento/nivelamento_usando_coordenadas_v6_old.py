...
lugar, a versão 4 implementa a função fim_curso. 
   ...
   usando o sensor magnético é muito impressiso, dessa forma vou mudar o algoritmo
...
######################################################################    
    ...
    def encontra_norte_magnetico(self):
        ...
        if defasagem_telescopio_sensor>0.2 or defasagem_telescopio_sensor<10:
            if defasagem_telescopio_sensor<0 or defasagem_telescopio_sensor>0.4:
                self.motor_horizontal.anti_horario(velocidade,(round (passos*1.3)))
                self.encontra_norte_magnetico()            
            elif defasagem_telescopio_sensor>350:
                self.motor_horizontal.horario(velocidade,passos)
                self.encontra_norte_magnetico()
            elif defasagem_telescopio_sensor>0 or defasagem_telescopio_sensor<0.4:
                print("TRUE")
                return True
        else:
            print("o valor do sensor é: ", defasagem_telescopio_sensor)                
